...
> [!IMPORTANT]
>
> La página **Repositorios** de Azure Sentinel se encuentra actualmente en **versión preliminar**. Consulte [Términos de uso complementarios para las Versiones preliminares de Microsoft Azure](https://azure.microsoft.com/support/legal/preview-supplemental-terms/) para conocer los términos legales adicionales que se aplican a las características de Azure que se encuentran en la versión beta, en versión preliminar o que todavía no se han publicado para que estén disponibles con carácter general.

El *contenido* de Azure Sentinel es contenido de Administración de eventos e información de seguridad (SIEM) que permite a los clientes a ingerir, supervisar, alertar, buscar y mucho más en Azure Sentinel. Por ejemplo, el contenido de Azure Sentinel incluye conectores de datos, analizadores, libros y reglas de análisis. Para obtener más información, consulte el artículo [Acerca de las soluciones y el contenido de Azure Sentinel](sentinel-solutions.md).

Puede aprovechar el contenido listo para usar (integrado) que se proporciona en el centro de contenido de Azure Sentinel y personalizarlo según sus necesidades, o bien crear su propio contenido personalizado desde cero.

A la hora de crear contenido personalizado, puede almacenarlo y administrarlo en sus propias áreas de trabajo de Azure Sentinel o en un repositorio de control de código fuente externo, incluidos los repositorios de GitHub y Azure DevOps. En este artículo se describe cómo crear y administrar las conexiones entre Azure Sentinel y los repositorios de control de código fuente externos. La administración del contenido en un repositorio externo permite actualizar dicho contenido fuera de Azure Sentinel e implementarlo automáticamente en las áreas de trabajo.

> [!TIP]
> En este artículo *no* se describe cómo crear tipos específicos de contenido desde cero. Para obtener más información, consulte la [wiki de GitHub de Azure Sentinel](https://github.com/Azure/Azure-Sentinel/wiki#get-started) pertinente para cada tipo de contenido.
>

## <a name="prerequisites-and-scope"></a>Requisitos previos y ámbito

Antes de conectar su área de trabajo de Azure Sentinel a un repositorio de control de código fuente externo, asegúrese de que tiene lo siguiente:
...
- Acceso a un repositorio de GitHub o Azure DevOps, con cualquier archivo de contenido personalizado que quiera implementar en sus áreas de trabajo en las [plantillas de Azure Resource Manager (ARM)](/azure/azure-resource-manager/templates/) pertinentes.
    ...
    Actualmente, Azure Sentinel solo admite conexiones con repositorios de GitHub y Azure DevOps.

- Un rol **Propietario** del grupo de recursos que contiene su área de trabajo de Azure Sentinel. El rol **Propietario** es necesario para crear la conexión entre Azure Sentinel y el repositorio de control de código fuente.

### <a name="maximum-connections-and-deployments"></a>Número máximo de conexiones e implementaciones

- Cada área de trabajo de Azure Sentinel está limitada actualmente a **cinco conexiones**.
...
La implementación de contenido en Azure Sentinel mediante la conexión con un repositorio no valida ese contenido, únicamente comprueba si los datos están en el formato de plantilla de ARM correcto.

Se recomienda validar las plantillas de contenido mediante el proceso de validación habitual. Puede usar las herramientas y el [proceso de validación de Azure Sentinel en GitHub](https://github.com/Azure/Azure-Sentinel/wiki#test-your-contribution) para configurar su propio proceso de validación.

## <a name="connect-a-repository"></a>Conexión de un repositorio

En este procedimiento se describe cómo conectar un repositorio de GitHub o Azure DevOps a su área de trabajo de Azure Sentinel, donde puede guardar y administrar su contenido personalizado, en lugar de en Azure Sentinel.

Cada conexión puede admitir varios tipos de contenido personalizado, como reglas de análisis, reglas de automatización, consultas de búsqueda, analizadores, cuadernos de estrategias y libros. Para obtener más información, consulte el artículo [Acerca de las soluciones y el contenido de Azure Sentinel](sentinel-solutions.md).
...
1. En Azure Sentinel, a la izquierda, en **Administración de contenido**, seleccione **Repositorios**.
...
1. Seleccione una de las pestañas siguientes según su tipo de conexión:
    ...
    1. Escriba sus credenciales de GitHub cuando se le solicite.
        ...
        La primera vez que agregue una conexión, se abrirá una nueva ventana o pestaña del explorador en la que le se pedirá que autorice la conexión a Azure Sentinel. Si ya ha iniciado sesión en su cuenta de GitHub en el mismo explorador, las credenciales de GitHub se rellenarán automáticamente.
    ...
    1. Una vez instalada la aplicación **Azure-Sentinel** en su repositorio, la lista desplegable **Rama** de la página **Crear una conexión nueva** se rellena automáticamente con sus ramas. Seleccione la rama a que quiere conectar a su área de trabajo de Azure Sentinel.

    1. En la lista desplegable **Tipo de contenido**, seleccione el tipo de contenido que va a implementar.

        - Tanto los analizadores como las consultas de búsqueda usan la API de **búsquedas guardadas** para implementar contenido en Azure Sentinel. Si selecciona uno de estos tipos de contenido y también tiene contenido del otro tipo en la rama, se implementarán ambos tipos de contenido.

        - Para el resto de tipos de contenido, al seleccionar uno en el panel **Crear una conexión nueva**, solo se implementa ese tipo de contenido en Azure Sentinel. No se implementan los otros tipos de contenido.
    ...
    Se inicia sesión automáticamente en Azure DevOps con sus credenciales actuales de Azure. Si no ha iniciado sesión en Azure DevOps con las mismas credenciales que usa en Azure Sentinel, cambie su cuenta de Azure DevOps para que coincida con la de Azure Sentinel.

    1.  En Azure Sentinel, en las listas desplegables que se muestran, seleccione la **organización**, el **proyecto**, el **repositorio**, la **rama** y el **tipo de contenido**.
    ...
    > [!NOTE]
    > No puede crear conexiones duplicadas, con el mismo repositorio y la misma rama, en una única área de trabajo de Azure Sentinel.
    >
...
- El contenido almacenado en el repositorio se muestra en el área de trabajo de Azure Sentinel, en la página correspondiente de Azure Sentinel.
...
Una vez que ha creado correctamente una conexión a su repositorio de control de código fuente, cada vez que se modifica el contenido de ese repositorio o se agrega nuevo contenido, el flujo de trabajo de implementación se ejecuta de nuevo e implementa todo el contenido del repositorio en todas las áreas de trabajo conectadas de Azure Sentinel.

Se recomienda editar cualquier contenido almacenado en un repositorio conectado *solo* en dicho repositorio y no en Azure Sentinel. Por ejemplo, para realizar cambios en las reglas de análisis, aplique dichos cambios directamente en GitHub o en Azure DevOps.

Si ha editado el contenido en Azure Sentinel, asegúrese de exportarlo al repositorio de control de código fuente para evitar que los cambios se sobrescriban la próxima vez que el contenido del repositorio se implemente en el área de trabajo.

Si va a eliminar contenido, asegúrese de eliminarlo tanto del repositorio como de Azure Portal. Al eliminar contenido del repositorio, este contenido no se elimina del área de trabajo de Azure Sentinel.

## <a name="remove-a-repository-connection"></a>Eliminación de la conexión a un repositorio

En este procedimiento se describe cómo quitar la conexión a un repositorio de control de código fuente en Azure Sentinel.
...
Una vez eliminada la conexión, el contenido que se implementó previamente mediante esta permanece en el área de trabajo de Azure Sentinel. El contenido agregado al repositorio después de quitar la conexión no se implementa.
...
### <a name="removing-the-azure-sentinel-app-from-your-github-repository"></a>Eliminación de la aplicación Azure Sentinel del repositorio de GitHub

Si piensa eliminar la aplicación Azure Sentinel de un repositorio de GitHub, se recomienda quitar *primero* todas las conexiones asociadas de la página **Repositorios** de Azure Sentinel.

Cada instalación de la aplicación Azure Sentinel tiene un identificador único que se usa al agregar y quitar la conexión. Si este identificador falta o se ha cambiado, deberá quitar la conexión de la página **Repositorios** de Azure Sentinel y también quitar manualmente el flujo de trabajo del repositorio de GitHub para evitar futuras implementaciones de contenido.
...
- [Analizadores del modelo de información de Azure Sentinel (ASIM) (versión preliminar pública)](normalization-about-parsers.md)
