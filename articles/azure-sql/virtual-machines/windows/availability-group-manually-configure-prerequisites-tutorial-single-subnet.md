---
title: 'Tutorial: Requisitos previos para un grupo de disponibilidad de una sola subred'
description: En este tutorial se muestra cómo configurar los requisitos previos para crear un grupo de disponibilidad AlwaysOn de SQL Server en Azure Virtual Machines en una sola subred.
...
En este tutorial se muestra cómo configurar los requisitos previos para crear un [grupo de disponibilidad AlwaysOn de SQL Server en Azure Virtual Machines](availability-group-manually-configure-tutorial-single-subnet.md) en una sola subred. Cuando haya completado los requisitos previos, tendrá un controlador de dominio, dos máquinas virtuales SQL Server y un servidor testigo en un único grupo de recursos.
...
:::image type="content" source="./media/availability-group-manually-configure-prerequisites-tutorial-single-subnet/00-EndstateSampleNoELB.png" alt-text="grupo de disponibilidad":::
...
2. Seleccione **+** para crear un nuevo objeto en el portal.
   ...
   :::image type="content" source="./media/availability-group-manually-configure-prerequisites-tutorial-single-subnet/01-portalplus.png" alt-text="Nuevo objeto":::

3. Haga clic en **grupo de recursos** en la ventana de búsqueda de **Marketplace**.

   :::image type="content" source="./media/availability-group-manually-configure-prerequisites-tutorial-single-subnet/01-resourcegroupsymbol.png" alt-text="Grupos de recursos":::
...
9. Compruebe que la función **Anclar al panel** está activada. Este parámetro opcional coloca un acceso directo para el grupo de recursos en el panel del Portal de Azure.
   ...
   :::image type="content" source="./media/availability-group-manually-configure-prerequisites-tutorial-single-subnet/01-resourcegroup.png" alt-text="Acceso directo del grupo de recursos en Azure Portal":::
...
## <a name="create-the-network-and-subnet"></a>Creación de la red y la subred

El siguiente paso es crear la red y subred en el grupo de recursos de Azure.

La solución usa una red virtual con una subred. En [Información general sobre redes virtuales](../../../virtual-network/virtual-networks-overview.md) encontrará más información sobre las redes de Azure.

Para crear la red virtual en Azure Portal:

1. En el grupo de recursos, seleccione **+ Agregar**. 

   :::image type="content" source="./media/availability-group-manually-configure-prerequisites-tutorial-single-subnet/02-newiteminrg.png" alt-text="Nuevo elemento":::
2. Busque **red virtual**.

     :::image type="content" source="./media/availability-group-manually-configure-prerequisites-tutorial-single-subnet/04-findvirtualnetwork.png" alt-text="Búsqueda de una red virtual":::
...
4. En **Red virtual**, seleccione el modelo de implementación de **Resource Manager** y, después, **Crear**.
   ...
   En el ejemplo se usa el nombre de subred **Admin**. Esta subred es para los controladores de dominio y las VM con SQL Server.

5. Seleccione **Crear**.

   :::image type="content" source="./media/availability-group-manually-configure-prerequisites-tutorial-single-subnet/06-configurevirtualnetwork.png" alt-text="Configuración de la red virtual":::
...
Después de haber creado la red, la subred y los conjuntos de disponibilidad, está preparado para crear las máquinas virtuales para los controladores de dominio.
...
1. En el portal, abra el grupo de recursos **SQL-HA-RG** y seleccione la máquina **ad-primary-dc**. En **ad-primary-dc**, seleccione **Conectar** para abrir un archivo RDP con el fin de acceder a Escritorio remoto.
    ...
    :::image type="content" source="./media/availability-group-manually-configure-prerequisites-tutorial-single-subnet/20-connectrdp.png" alt-text="Conexión a una máquina virtual":::
...
4. Seleccione el vínculo **Agregar roles y características** en el panel.
    ...
    :::image type="content" source="./media/availability-group-manually-configure-prerequisites-tutorial-single-subnet/22-add-features.png" alt-text="Seleccione el vínculo Agregar roles y características en el panel.":::
...
6. Seleccione los roles **Active Directory Domain Services** y **Servidor DNS**. Cuando se le pida, agregue características adicionales que son necesarias para estos roles.
   ...
   >
    ...
    :::image type="content" source="./media/availability-group-manually-configure-prerequisites-tutorial-single-subnet/23-add-roles.png" alt-text="Seleccione los roles Active Directory Domain Services y Servidor DNS.":::
...
11. Seleccione el vínculo **Más** en la barra de advertencia amarilla.
    ...
    :::image type="content" source="./media/availability-group-manually-configure-prerequisites-tutorial-single-subnet/24-addsmore.png" alt-text="Cuadro de diálogo AD DS en la máquina virtual del servidor DNS":::
...
:::image type="content" source="./media/availability-group-manually-configure-prerequisites-tutorial-single-subnet/25-primarydcip.png" alt-text="Interfaces de red":::
...
4. En **Centro de redes y recursos compartidos**, seleccione la interfaz de red.
   ...
   :::image type="content" source="./media/availability-group-manually-configure-prerequisites-tutorial-single-subnet/26-networkinterface.png" alt-text="Interfaz de red":::
...
10. Seleccione el vínculo **Agregar roles y características** en el panel.
    ...
    :::image type="content" source="./media/availability-group-manually-configure-prerequisites-tutorial-single-subnet/22-add-features.png" alt-text="Selección del vínculo Agregar roles y características en el panel.":::
...
17. En **Configuración de implementación**, seleccione **Agregar un controlador de dominio a un dominio existente**.
    ...
    :::image type="content" source="./media/availability-group-manually-configure-prerequisites-tutorial-single-subnet/28-deploymentconfig.png" alt-text="Configuración de la implementación":::
...
4. En el panel **Tareas** de la derecha, seleccione **Nuevo** y, después, **Usuario**.
   ...
   :::image type="content" source="./media/availability-group-manually-configure-prerequisites-tutorial-single-subnet/29-addcnewuser.png" alt-text="Centro de administración de Active Directory":::
...
1. En el **Centro de administración de Active Directory**, seleccione **corp (local)** en el panel izquierdo. Luego, en el panel **Tareas** a la derecha, seleccione **Propiedades**.
    ...
    :::image type="content" source="./media/availability-group-manually-configure-prerequisites-tutorial-single-subnet/31-addcproperties.png" alt-text="Propiedades de usuario CORP":::
...
6. Active la casilla **Create Computer objects** (Crear objetos de proceso).
     ...
     :::image type="content" source="./media/availability-group-manually-configure-prerequisites-tutorial-single-subnet/33-addpermissions.png" alt-text="Permisos de usuario Corp":::
...
> [!NOTE]
> Los tamaños de máquina que se sugieren aquí están diseñados para probar los grupos de disponibilidad en Azure Virtual Machines. Para optimizar el rendimiento de las cargas de trabajo de producción, consulte las recomendaciones de tamaños de máquina y la configuración de SQL Server en [Procedimientos recomendados de SQL Server en Azure Virtual Machines](./performance-guidelines-best-practices-checklist.md?toc=%2fazure%2fvirtual-machines%2fwindows%2ftoc.json).
>
...
4. Para el puerto, especifique **TCP** y escriba los números de puerto adecuados. Observe el ejemplo siguiente:
   ...
   :::image type="content" source="./media/availability-group-manually-configure-prerequisites-tutorial-single-subnet/35-tcpports.png" alt-text="Firewall de SQL":::
...
Ahora que ha configurado los requisitos previos, comience con la [configuración del grupo de disponibilidad](availability-group-manually-configure-tutorial-single-subnet.md).
