...
> * [Ha configurado el entorno de desarrollo y ha creado un proyecto vacío](~/articles/cognitive-services/speech-service/quickstarts/setup-platform.md?tabs=uwp&pivots=programming-language-csharp)
...
1. En la vista XAML del diseñador, reemplace todo el contenido por el siguiente fragmento de código, que define una interfaz de usuario rudimentaria:
...
1. En **Explorador de soluciones**, abra el archivo de código fuente subyacente `MainPage.xaml.cs`. (Se agrupa en `MainPage.xaml`). Reemplace el contenido de este archivo por el siguiente, que incluye:
...
1. Reemplace las cadenas `YourSpeechSubscriptionKey` y `YourServiceRegion` por sus valores para la suscripción de voz y la [región](~/articles/cognitive-services/speech-service/regions.md).
...
1. Elija **Depurar** > **Iniciar depuración** o presione **F5** para iniciar la aplicación. Aparece la ventana **HelloWorld**.
   ...
   ![Aplicación de asistente de voz para UWP de ejemplo en C#: inicio rápido](~/articles/cognitive-services/Speech-Service/media/sdk/qs-voice-assistant-uwp-helloworld-window.png)
