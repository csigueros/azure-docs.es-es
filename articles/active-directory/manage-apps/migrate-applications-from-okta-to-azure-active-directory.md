...
1. En el panel de API, en la sección **Seguridad**, seleccione **Tokens** > **Crear token**.
...
1. Inserte un nombre de token y seleccione **Crear token**.
...
1. Registre el valor del token y guárdelo. Una vez que seleccione **De acuerdo**, ya no será accesible.

   ![Captura de pantalla en la que se muestra el cuadro Valor del token.](media/migrate-applications-from-okta-to-azure-active-directory/record-created.png)

1. En la aplicación Postman, en el área de trabajo, seleccione **Importar**.
...
1. En la página **Importar**, seleccione **Vincular**. A continuación, inserte el vínculo siguiente para importar la API: `https://developer.okta.com/docs/api/postman/example.oktapreview.com.environment`
...
1. A continuación, seleccione **Importar**.
...
1. Una vez realizada la importación de la API, cambie la selección de **entorno** a **{suDominiodeOkta}** .
...
1. Para editar el entorno de Okta, seleccione el icono de ojo. Después, seleccione **Editar**.
...
1. Actualice los valores de la URL y la clave de API en los campos **Valor inicial** y **Valor actual**. Cambie el nombre para que se refleje el entorno. A continuación, guarde los valores.
...
1. [Cargue la API en Postman](https://app.getpostman.com/run-collection/377eaf77fdbeaedced17).

1. Seleccione **Aplicaciones** > **Obtener aplicaciones de lista** > **Enviar**.

   Ahora ya puede imprimir todas las aplicaciones del inquilino de Okta. La lista está en formato JSON.
...
1. En la **galería de Azure AD**, busque la aplicación **Salesforce**, selecciónela y, a continuación, seleccione **Crear**.
...
1. Una vez creada la aplicación, en la pestaña **Inicio de sesión único**, seleccione **SAML**.
...
1. Descargue el **Certificado (sin procesar)** y **XML de metadatos de federación** para importarlos en Salesforce.
...
1. En la consola de administración de Salesforce, seleccione **Identidad** > **Configuración de inicio de sesión único** > **Nuevo archivo de metadatos**.
...
1. Cargue el archivo XML descargado del portal de Azure AD. Seleccione **Crear**.
...
1. Cargue el certificado que descargó de Azure. A continuación, seleccione **Guardar** para crear el proveedor SAML en Salesforce.
...
1. Guarde los valores de los campos siguientes. Usará estos valores en Azure.
...
1. Vuelva a la página **Aplicaciones empresariales** de Azure AD y, en la configuración de SSO de SAML, seleccione **Cargar el archivo de metadatos** para cargar el archivo en el portal de Azure AD. Antes de guardar, asegúrese de que los valores importados coinciden con los valores registrados.
...
1. En la consola de administración de Salesforce, seleccione **Configuración de la compañía** > **Mi dominio**. Vaya a **Configuración de autenticación** y seleccione **Editar**.
...
1. Para una opción de inicio de sesión, seleccione el nuevo proveedor SAML que configuró anteriormente. Después, seleccione **Guardar**.
...
1. En Azure AD, en la página **Aplicaciones empresariales**, seleccione **Usuarios y grupos**. A continuación, agregue usuarios de prueba.
...
1. Para probar la configuración, inicie sesión como uno de los usuarios de prueba. Vaya a la [galería de aplicaciones](https://aka.ms/myapps) de Microsoft y seleccione **Salesforce**.
...
1. Seleccione el proveedor de identidades (IdP) recién configurado para iniciar sesión.
...
1. En la consola de administración de Salesforce, seleccione **Configuración de la compañía** > **Mi dominio**.

1. En **Configuración de autenticación**, seleccione **Editar**. Desactive la selección de **Okta** como servicio de autenticación.
...
1. Seleccione **Cree su propia aplicación**. En el menú que aparece, asigne un nombre a la aplicación de OIDC y seleccione **Registrar una aplicación con la que está trabajando para integrarla con Azure AD**. Seleccione **Crear**.
...
1. En la página siguiente, configure los inquilinos del registro de la aplicación. Para más información, consulte [Inquilinos en Azure Active Directory](../develop/single-and-multi-tenant-apps.md).
...
1. En la página **Registros de aplicaciones**, en **Azure Active Directory**, abra el registro recién creado.
...
1. En la página **Información general**, guarde el **identificador de aplicación (cliente)** . Usará este identificador en la aplicación.
...
1. A la izquierda, seleccione **Certificados y secretos**. A continuación, seleccione **Nuevo secreto de cliente**. Asigne al secreto de cliente el nombre y establezca su expiración.
...
1. Para mejorar la experiencia de usuario y suprimir las solicitudes de consentimiento del usuario, seleccione la opción **Grant admin consent for Tenant Domain Name** (Conceder consentimiento de administrador para nombre de dominio de inquilino) A continuación, espere a que aparezca el estado **Concedido**.
...
1. En la aplicación configurada con OIDC, importe el identificador de la aplicación y el secreto de cliente antes de realizar la prueba. Siga los pasos anteriores para configurar la aplicación con valores como el identificador de cliente, el secreto y los ámbitos.
