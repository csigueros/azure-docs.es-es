...
> [!IMPORTANT]
> El almacenamiento inmutable para Azure Blob Storage en cuentas que tienen habilitada la característica de espacio de nombres jerárquico se encuentra actualmente en versión preliminar.
...
Sí, si los requisitos de cumplimiento permiten habilitar la eliminación temporal. La [eliminación temporal para Azure Blob Storage](./soft-delete-blob-overview.md) se aplica a todos los contenedores dentro de una cuenta de almacenamiento, con independencia de que haya vigente una directiva de retención con duración definida o una suspensión legal. Microsoft recomienda habilitar la eliminación temporal para mayor protección antes de aplicar directivas de inmutabilidad. Si la eliminación temporal está habilitada en un contenedor y, después, se agrega una directiva de inmutabilidad al contenedor, los blobs que ya se hayan eliminado temporalmente se eliminarán de forma permanente una vez que haya expirado la directiva de retención de eliminación temporal. Los blobs eliminados temporalmente se pueden restaurar durante el período de retención de eliminación temporal. Los blobs que aún no se han eliminado temporalmente están protegidos por la directiva de inmutabilidad y no se pueden eliminar temporalmente hasta que la directiva expire (en el caso de las retenciones con duración definida) o se quite (en el caso de las suspensiones legales).
