...
ms.locfileid: "131012328"
---
# <a name="webhook-actions-for-log-alert-rules"></a>Acciones de webhook para reglas de alertas de registro

La [alerta de registro](alerts-log.md) admite la [configuración de grupos de acciones de webhook](./action-groups.md#webhook). En este artículo, describiremos qué propiedades están disponibles. Las acciones de webhook permiten invocar una única solicitud HTTP POST. El servicio al que se llama debe admitir webhooks y saber cómo usar la carga que reciba.
...
### <a name="log-alert-for-all-resources-logs-from-api-version-2020-08-01"></a>Alerta de registro para todos los registros de recursos (a partir de la versión `2020-08-01` de la API)

La carga de ejemplo siguiente corresponde a un webhook estándar cuando se usa para alertas de registro basadas en registros de recursos:
...
### <a name="log-alert-for-log-analytics-up-to-api-version-2018-04-16"></a>Alerta de registro para Log Analytics (hasta la versión `2018-04-16` de la API)
...
### <a name="log-alert-for-application-insights-up-to-api-version-2018-04-16"></a>Alerta de registro para Application Insights (hasta la versión `2018-04-16` de la API)
...
### <a name="log-alert-with-a-custom-json-payload-up-to-api-version-2018-04-16"></a>Alerta de registro con una carga JSON personalizada (hasta la versión `2018-04-16` de la API)

> [!NOTE]
> El webhook basado en JSON personalizado no se admite desde la versión `2021-08-01` de la API.
...
Puede usar la opción **Incluir la carga personalizada de JSON para webhook** para obtener una carga de JSON personalizada mediante los parámetros. También puede generar propiedades adicionales.
...
```
Las variables de un webhook personalizado se deben especificar dentro de un contenedor JSON. Por ejemplo, si se hace referencia a "#searchresultcount" en el ejemplo de webhook, se generará la salida en función de los resultados de la alerta.
