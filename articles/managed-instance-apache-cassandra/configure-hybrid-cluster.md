...
1. Ahora aplicaremos algunos permisos especiales a la red virtual y a la subred que requiere Managed Instance for Apache Cassandra mediante la CLI de Azure. Utilice el comando `az role assignment create` y reemplace `<subscriptionID>`, `<resourceGroupName>` y `<vnetName>` con los valores adecuados:
