---
title: 'Limpiar datos que faltan: referencia de componente'
titleSuffix: Azure Machine Learning
description: Aprenda a usar el componente Limpiar datos que faltan de Azure Machine Learning para quitar, reemplazar o deducir valores que faltan.
...
ms.locfileid: "131566011"
---
# <a name="clean-missing-data-component"></a>Componente Limpiar datos que faltan

En este artículo se describe un componente del diseñador de Azure Machine Learning.

Use este componente para quitar, reemplazar o deducir valores que faltan. 
...
Este componente admite varios tipos de operaciones para "limpiar" los valores que faltan, que incluyen:
...
El uso de este componente no cambia el conjunto de datos de origen. En su lugar, crea un nuevo conjunto de datos en el área de trabajo que puede usar en el flujo de trabajo posterior. También puede guardar el conjunto de datos nuevo y limpio para reutilizarlo.

Este componente también genera una definición de la transformación usada para limpiar los valores que faltan. Puede volver a usar esta transformación en otros conjuntos de datos que tengan el mismo esquema con el componente [Aplicación de la transformación](./apply-transformation.md).  

## <a name="how-to-use-clean-missing-data"></a>Procedimiento para usar Limpiar datos que faltan

Este componente permite definir una operación de limpieza. También puede guardar la operación de limpieza para aplicarla más adelante a nuevos datos. Vea las secciones siguientes para obtener una descripción de cómo crear y guardar un proceso de limpieza: 
...
Cada vez que se aplica el componente [Limpiar datos que faltan](./clean-missing-data.md) a un conjunto de datos, se aplica la misma operación de limpieza a todas las columnas que se seleccionen. Por lo tanto, si tiene que limpiar distintas columnas mediante diferentes métodos, use instancias independientes del componente.

1.  Agregue el componente [Limpiar datos que faltan](./clean-missing-data.md) a la canalización y conecte el conjunto de datos al que le faltan valores.  
...
2.  Para **Columns to be cleaned** (Columnas que se deben limpiar), elija las columnas que contienen los valores que faltan que desee cambiar. Puede elegir varias columnas, pero debe usar el mismo método de reemplazo en todas las columnas seleccionadas. Por lo tanto, normalmente deberá limpiar las columnas de cadena y las columnas numéricas por separado.
    ...
    1. Seleccione el componente **Limpiar datos que faltan** y haga clic en **Editar columna** en el panel derecho del componente.

    3. Para **Incluir**, seleccione **Tipos de columna** en la lista desplegable y, a continuación, seleccione **Numérico**. 
  
    Cualquier método de limpieza o de reemplazo que elija debe ser aplicable a **todas** las columnas de la selección. Si los datos de cualquier columna no son compatibles con la operación especificada, el componente devuelve un error y detiene la canalización.
...
1.  Agregue el componente [Aplicación de la transformación](./apply-transformation.md) a la canalización.  
