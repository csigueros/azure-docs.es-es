---
title: Uso de la biblioteca cliente de Form Recognizer para Python
...
Cree una nueva aplicación de Python con el nombre `form-recognizer.py` en el IDE o editor que prefiera. A continuación, importe las bibliotecas siguientes.
...
* El entrenamiento de modelos personalizados para analizar todos los campos y los valores que se encuentren en los formularios personalizados. Vea la [documentación del servicio sobre el entrenamiento de modelos sin etiquetar](#train-a-model-without-labels).
* El entrenamiento de modelos personalizados para analizar los campos y los valores concretos que especifique mediante la etiqueta de los formularios personalizados. Vea la [documentación del servicio sobre el entrenamiento de modelos etiquetados](#train-a-model-with-labels).
...
> [!TIP]
 > También puede analizar imágenes de documentos de identificación con los métodos [FormRecognizerClient](/python/api/azure-ai-formrecognizer/azure.ai.formrecognizer.formrecognizerclient?view=azure-python&preserve-view=true#methods), como `begin_recognize_identity_documents`. 
...
Esta es la salida de un modelo entrenado con los datos de entrenamiento disponibles en el [SDK de Python](https://github.com/Azure/azure-sdk-for-python/tree/master/sdk/formrecognizer/azure-ai-formrecognizer/samples/sample_forms/training).
...
> [!IMPORTANT]
> Para realizar el entrenamiento con etiquetas, debe disponer de archivos de información con etiquetas especiales (`\<filename\>.pdf.labels.json`) en el contenedor de almacenamiento de blobs junto con los documentos para el entrenamiento. La [herramienta de etiquetado de ejemplo de Form Recognizer](../../label-tool.md) proporciona una interfaz de usuario para ayudar a crear estos archivos de etiquetas. Cuando los tenga, puede llamar a la función `begin_training` con el parámetro *use_training_labels* establecido en `true`.
...
Esta es una salida de ejemplo de la cuenta de prueba.
...
Esta es la salida de ejemplo del modelo personalizado creado en el ejemplo anterior.
...
Ejecute la aplicación con el comando `python` siguiente:
...
En este proyecto se ha usado la biblioteca cliente de Form Recognizer para Python para entrenar modelos y analizar formularios de maneras diferentes. A continuación, obtenga sugerencias para crear un mejor conjunto de datos de entrenamiento y generar modelos más precisos.
...
* El código de ejemplo de este proyecto se puede encontrar en [GitHub](https://github.com/Azure-Samples/cognitive-services-quickstart-code/blob/master/python/FormRecognizer/FormRecognizerQuickstart.py).
