...
| `SCM_TRACE_LEVEL` | Nivel de seguimiento de compilación. De manera predeterminada, es `1`. Establézcalo en valores superiores, hasta 4, para un seguimiento mayor. | `4` |
...
| `SCM_SITEEXTENSIONS_FEED_URL` | Dirección URL de la galería de extensiones de sitio. De manera predeterminada, es `https://www.nuget.org/api/v2/`. La dirección URL de la fuente anterior es `http://www.siteextensions.net/api/v2/`. | |
...
| `WEBSITE_SCM_IDLE_TIMEOUT_IN_MINUTES` | Tiempo de espera en minutos para el sitio de SCM (Kudu). De manera predeterminada, es `20`. | |
...
| `WEBSITE_SLOT_NUMBER_OF_TIMEOUTS_BEFORE_RESTART` | Durante un intercambio de ranura, el número máximo de tiempos de espera expirados tras los cuales se fuerza el reinicio del sitio en una instancia de máquina virtual específica. De manera predeterminada, es `3`. ||
| `WEBSITE_SLOT_MAX_NUMBER_OF_TIMEOUTS` | Durante un intercambio de ranura, número máximo de solicitudes de tiempo de espera para una única dirección URL que se debe realizar antes de desistir. De manera predeterminada, es `5`. ||
| `WEBSITE_SKIP_ALL_BINDINGS_IN_APPHOST_CONFIG` | Establezca el valor en `true` o `1` para omitir todos los enlaces en el archivo `applicationHost.config`. De manera predeterminada, es `false`. Si la aplicación desencadena un reinicio porque `applicationHost.config` se ha actualizado con los nombres de host de las ranuras intercambiados, establezca esta variable en `true` para evitar un reinicio de este tipo. Si ejecuta una aplicación de Windows Communication Foundation (WCF), no establezca esta variable. ||
...
| `WEBSITES_PORT` | Para un contenedor personalizado, el número de puerto personalizado del contenedor al que se van a enrutar las solicitudes. De manera predeterminada, App Service intenta la detección automática de los puertos 80 y 8080. ||
...
| `MACHINEKEY_Decryption` | Para contenedores Windows, esta variable se inserta en el contenedor para habilitar las rutinas criptográficas de ASP.NET (consulte [Elemento machineKey (Esquema de configuración de ASP.NET)](/previous-versions/dotnet/netframework-4.0/w8h3skw9(v=vs.100))). Para invalidar el valor predeterminado de `decryption`, establezca este valor como una configuración de la aplicación. ||
| `MACHINEKEY_DecryptionKey` | Para contenedores Windows, esta variable se inserta en el contenedor para habilitar las rutinas criptográficas de ASP.NET (consulte [Elemento machineKey (Esquema de configuración de ASP.NET)](/previous-versions/dotnet/netframework-4.0/w8h3skw9(v=vs.100))). Para invalidar el valor generado automáticamente de `decryptionKey`, establezca este valor como una configuración de la aplicación. ||
| `MACHINEKEY_Validation` | Para contenedores Windows, esta variable se inserta en el contenedor para habilitar las rutinas criptográficas de ASP.NET (consulte [Elemento machineKey (Esquema de configuración de ASP.NET)](/previous-versions/dotnet/netframework-4.0/w8h3skw9(v=vs.100))). Para invalidar el valor predeterminado de `validation`, establezca este valor como una configuración de la aplicación. ||
| `MACHINEKEY_ValidationKey` | Para contenedores Windows, esta variable se inserta en el contenedor para habilitar las rutinas criptográficas de ASP.NET (consulte [Elemento machineKey (Esquema de configuración de ASP.NET)](/previous-versions/dotnet/netframework-4.0/w8h3skw9(v=vs.100))). Para invalidar el valor generado automáticamente de `validationKey`, establezca este valor como una configuración de la aplicación. ||
...
| `DIAGNOSTICS_LASTRESORTFILE` | Nombre del archivo que se va a crear, o una ruta de acceso relativa al directorio de registro, para registrar los errores internos para solucionar problemas del cliente de escucha. De manera predeterminada, es `logging-errors.txt`. ||
| `DIAGNOSTICS_LOGGINGSETTINGSFILE` | Ruta de acceso al archivo de configuración de registro, relativa a `D:\home` o `/home`. De manera predeterminada, es `site\diagnostics\settings.json`. | |
...
| `DIAGNOSTICS_TEXTTRACEMAXNUMLOGFILES` | Número máximo de archivos de registro que se van a conservar. De manera predeterminada, es `20`. | |
...
| `WEBJOBS_HISTORY_SIZE`| Para los trabajos desencadenados, es el número máximo de ejecuciones que se mantienen en el directorio del historial por cada trabajo. De manera predeterminada, es `50`. |
