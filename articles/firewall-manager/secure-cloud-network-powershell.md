...
- PowerShell 7
   ...
   En este tutorial, es preciso ejecutar Azure PowerShell de forma local en PowerShell 7. Para instalar PowerShell 7, consulte [Migración de Windows PowerShell 5.1 a PowerShell 7](/powershell/scripting/install/migrating-from-windows-powershell-51-to-powershell-7?view=powershell-7&preserve-view=true).
...
```
> [!NOTE]
> La cadena "***all_traffic***" como valor del parámetro "-Name" en el comando New-AzVHubRoute tiene un significado especial: si usa esta cadena exacta, la configuración aplicada en este artículo se reflejará correctamente en Azure Portal (Firewall Manager --> Centros virtuales --> [Su centro] --> Configuración de seguridad). Si se va a usar otro nombre, se aplicará la configuración deseada, pero no se reflejará en Azure Portal. 
