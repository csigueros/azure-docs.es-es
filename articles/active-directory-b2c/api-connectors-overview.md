...
description: Use conectores de API de Azure Active Directory (Azure AD) para personalizar y ampliar los flujos de usuario mediante API REST o webhooks de salida a orígenes de datos de identidad externos.
...
ms.locfileid: "131436942"
---
# <a name="use-api-connectors-to-customize-and-extend-sign-up-user-flows-with-external-identity-data-sources"></a>Uso de conectores de API para personalizar y ampliar los flujos de usuario de registro con orígenes de datos de identidad externos 
...
- **Compruebe la identidad del usuario**. Use un servicio de verificación de identidad u orígenes de datos de identidad externos para agregar un nivel adicional de seguridad a las decisiones de creación de cuentas.
...
- **Usar el origen de datos de identidad externo para validar los datos de entrada del usuario**. Por ejemplo, puede verificar que la dirección de correo electrónico que proporciona el usuario existe en la base de datos del cliente y, en caso contrario, presentar un error. También puede pensar en los conectores de API como una forma de admitir webhooks salientes porque la llamada se realiza cuando se produce un evento, por ejemplo, un registro.
- **Procesar notificaciones**. Si un usuario escribe su nombre todo con mayúsculas o minúsculas, la API de REST puede dar formato al nombre para que solo la primera letra esté en mayúscula y lo devuelva a Azure AD B2C. Sin embargo, cuando se usa una directiva personalizada, se prefiere [ClaimsTransformations](claimstransformations.md) en lugar de llamar a una API RESTful. 
- **Enriquecer dinámicamente datos de usuario mediante integración adicional en aplicaciones de línea de negocio corporativas**. el servicio RESTful puede recibir la dirección de correo electrónico del usuario, consultar la base de datos del cliente y devolver el número de fidelidad del usuario a Azure AD B2C. Luego, las notificaciones devueltas pueden almacenarse en la cuenta de Azure AD del usuario, evaluarse en los siguientes pasos de orquestación o incluirse en el token de acceso.
